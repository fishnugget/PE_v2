...
class ImageHelper extends AppHelper{
    ...
    public function resize($image, $width, $height, $options = array()){
		...
		if(empty($height)){
			$GDimage = imagecreatefromjpeg(WWW_ROOT.$image);
			$orig_width = imagesx($GDimage);
			$orig_height = imagesy($GDimage);
			//debug($image);
			// Calc the new height
    ...
    public function resizedUrl($file, $width, $height){
        ...
        $output     = $pathinfo['dirname'] . '/' . $pathinfo['filename'] . '_' . $width . 'W' . '.' . $pathinfo['extension'];
